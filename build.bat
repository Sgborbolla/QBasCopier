...
move /y "dist\windows-tmp\QBasCopier.exe" "dist\Windows\QBasWing-Shuttle.exe" >nul || goto :err
...
move /y "dist\setup-tmp\QBasCopierSetup.exe" "dist\Windows\QBasWing-Shuttle Setup.exe" >nul || goto :err
...
echo    - "QBasWing-Shuttle.exe"        (portable, listo para copiar a PC)
echo    - "QBasWing-Shuttle Setup.exe"  (instalador con musica e idioma)
echo   Linux -^> dist\Linux\   macOS -^> dist\macOS\  (build.sh / make-app.sh)
echo   Android -^> dist\QBasWing-Shuttle-v1.1.apk  (build-android.sh)
echo  ============================================================
echo.
pause
exit /b 0
