#!/usr/bin/env bash
# QBasCopier y Transfer - build.sh
# Uso:
...
# Salida: dist/Linux/QBasCopier y Transfer  o  dist/macOS/QBasCopier y Transfer
set -euo pipefail
cd "$(dirname "$0")"
...
echo "[1/3] Publicando QBasCopier y Transfer ($RID, self-contained)..."
mkdir -p "dist/$SYS"
...
mv "dist/$SYS/QBasCopier" "dist/$SYS/QBasCopier-y-Transfer" 2>/dev/null || true
echo "[2/3] LISTO: dist/$SYS/QBasCopier-y-Transfer"

if [ "$MODE" = "install" ]; then
  echo "[3/3] Instalando en ~/.local ..."
  mkdir -p "$HOME/.local/bin"
  cp "dist/$SYS/QBasCopier-y-Transfer" "$HOME/.local/bin/QBasCopier-y-Transfer"
  chmod +x "$HOME/.local/bin/QBasCopier-y-Transfer"
